...
- ~~**Retour en arrière (2026-09-22) : le périmètre par défaut redevient le label `automatedcreation` seul**~~ — **annulé le même jour, voir ci-dessous.**
- **PÉRIMÈTRE DÉFINITIF (2026-09-22, après confirmation du terrain par l'utilisateur).** Les deux tentatives précédentes (filtrer par label `automatedcreation`, puis par Request Type + date du 11 juin) étaient **fausses, toutes les deux**, parce qu'elles reposaient sur une compréhension erronée : je lisais « pas de label `automatedcreation` » comme « hors périmètre ». La réalité du terrain :
  - **Trois canaux d'entrée alimentent la même file**, et tous font partie du travail de l'équipe : One Portail (label `automatedcreation`, création directe depuis le portail), JSD, et le dashboard PECARTES. Les labels produit/domaine (`SaaS_Cartes`, `EEC-*`, `micro`, `macro`…) viennent des billets JSD/dashboard — ce ne sont pas des marqueurs d'origine mais de sujet.
  - **Le canal est une information à afficher, jamais un critère d'exclusion.**
  - **La vraie borne est temporelle** : la migration du **12 septembre 2026**, bascule vers le système actuel. Avant, autre monde.
  - **Conséquence dans `analyser.py`** : le périmètre est une fenêtre de temps (12 mois par défaut, `--mois N`, `--depuis-migration`, `--tous`). Les options `--large` et `--directs` sont supprimées, devenues sans objet. Le rapport affiche la répartition par canal et marque la semaine de la migration.
  - **Distinction à garder en tête** : le *rapport* a besoin de profondeur historique (tendances) ; l'*outil de triage* travaille sur le post-migration (260 billets). Ce ne sont pas le même périmètre, et c'est normal.
...
- [x] Relancé sur le périmètre label (195 billets) et relu.
...
- [x] **Livrable 1 fermé (2026-09-22), puis refait sur le bon périmètre le même jour** (voir la révision de périmètre ci-dessous). Les chiffres calculés sur le périmètre `automatedcreation` seul (196 billets, 98 fermés) sont **obsolètes et étaient trompeurs** — échantillon trop petit : il donnait 17,3 % de réaffectation contre 8,5 % réellement.
- [x] **Référence actuelle, chiffres définitifs (12 mois, 3507 billets dont 3168 fermés)** :
  - **8,5 %** réaffectés au moins une fois (269 billets)
  - **35,7 %** avec priorité modifiée après ouverture (1131 billets) ← le chiffre de la démo
  - **4,3 %** réouverts après fermeture (136 billets)
  - **Ces chiffres sont validés par recoupement** : le périmètre « large » (755 fermés, échantillon indépendant) donnait 8,3 % / 35,8 % / 3,7 %. Deux échantillons très différents, mêmes résultats — ce sont des caractéristiques structurelles du processus, pas du bruit.
- [x] **La migration a inversé les canaux d'entrée** (constat présentable) : sur 12 mois, JSD/dashboard 2496 (71 %), création directe 815 (23 %), One Portail 196 (6 %). Depuis la migration : One Portail 164 (63 %), JSD/dashboard 81 (31 %), création directe 15 (6 %). One Portail est passé de marginal à dominant en dix jours.
- [x] **Contrainte de volume levée** : 3168 billets fermés disponibles sur 12 mois — la taxonomie (~200) et le jeu d'évaluation (120) ne sont plus limités par la taille du corpus.
...
- [x] **Premier test réel fait (2026-09-22), sur PECARTES-21976.** Résultat encourageant : `PRIORITE_CRITERE` écrit dans le style observable de `severite.md` (pas d'adjectif) ; `CATEGORIE: changement-implementation` (une des deux catégories laissées ambiguës dans `categories.md`) — le billet (transfert de fichiers SGCI *planifié*, pas un échec) aide à la distinguer de `fichier-non-reçu` (un transfert qui *échoue*) ; `DEJA_VU: PECARTES-25080` **vérifié par l'utilisateur — le billet existe et c'est un vrai doublon**, pas un numéro inventé. Encore informel (un seul billet, pas le jeu d'évaluation à 120), mais la chaîne de bout en bout fonctionne et le signal `DEJA_VU` semble fiable, pas juste une case remplie pour la forme.
- [ ] Compléter la fiche `changement-implementation` dans `categories.md` avec cet exemple, plutôt que de la laisser dans la liste « ambiguë, à trancher ».
